...
async function pageCampanhas(){
  ...
  const isAdmin = me && me.role === 'ADMIN_MASTER';
  ...
  wrap.innerHTML = `
    ...
    ${isAdmin ? `
      <div style="height:12px"></div>
      <div class="card" style="padding:16px">
        <h3 style="margin:0 0 10px">Criar campanha (Admin Master)</h3>
        <form id="campCreate" class="grid" style="grid-template-columns: 1fr 1fr; gap:12px">
          <div>
            <label>Título</label>
            <input id="c_titulo" required placeholder="Ex: Rifa do Congresso" />
          </div>
          <div>
            <label>Valor da cota (R$)</label>
            <input id="c_valor" required placeholder="Ex: 10,00" />
          </div>
          <div style="grid-column:1/-1">
            <label>Descrição</label>
            <textarea id="c_desc" rows="3" placeholder="Prêmio, regras, data do sorteio..."></textarea>
          </div>
          <div>
            <label>Total de cotas</label>
            <input id="c_total" required type="number" min="1" value="100" />
          </div>
          <div>
            <label>Tempo de reserva (min)</label>
            <input id="c_reserva" required type="number" min="5" value="30" />
          </div>
          <div style="grid-column:1/-1">
            <button class="btn" type="submit">Criar e gerar cotas</button>
          </div>
          <div class="small">Depois: use <b>Adicionar vendedor</b> e divulgue o link público.</div>
        </form>
      </div>
    ` : ''}
    ...
    ${list.map(c=>`
      <div class="card" style="padding:16px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap">
          <div>
            <h3 style="margin:0 0 6px">${escapeHtml(c.titulo)}</h3>
            <div class="small">${escapeHtml(c.descricao||'')}</div>
          ...
          <div style="display:flex;gap:10px;align-items:flex-start;flex-wrap:wrap">
            ...
            ${isAdmin ? `
              <button class="btn" data-act="seller" data-id="${c.id}">Adicionar vendedor</button>
              <button class="btn" data-act="gen" data-id="${c.id}">Gerar cotas</button>
            ` : ''}
          </div>
        </div>
      </div>
  ...
  if(isAdmin){
    qs('#campCreate')?.addEventListener('submit', async (ev)=>{
      ev.preventDefault();
      try{
        const valorTxt = qs('#c_valor').value.replace('.', '').replace(',', '.');
        const valor = Math.round(Number(valorTxt) * 100);
        const body = {
          titulo: qs('#c_titulo').value.trim(),
          descricao: qs('#c_desc').value.trim(),
          valor_cota_cents: valor,
          total_cotas: Number(qs('#c_total').value),
          reserva_minutos: Number(qs('#c_reserva').value),
          modelo_cotas: 'SEQUENTIAL'
        };
        const created = await api('/api/campanhas', { method:'POST', auth:true, body });
        await api(`/api/campanhas/${created.id}/gerar-cotas`, { method:'POST', auth:true });
        toast('Campanha criada e cotas geradas', 'success');
        window.location.reload();
      }catch(e){ toast(e.message||'Erro ao criar campanha','error'); }
    });

    wrap.querySelectorAll('button[data-act="gen"]').forEach(btn=>btn.addEventListener('click', async ()=>{
      const id = btn.getAttribute('data-id');
      if(!confirm('Gerar/garantir cotas desta campanha?')) return;
      try{ btn.disabled=true; await api(`/api/campanhas/${id}/gerar-cotas`, { method:'POST', auth:true }); toast('Cotas ok','success'); }
      catch(e){ toast(e.message||'Erro ao gerar cotas','error'); }
      finally{ btn.disabled=false; }
    }));

    wrap.querySelectorAll('button[data-act="seller"]').forEach(btn=>btn.addEventListener('click', async ()=>{
      const id = btn.getAttribute('data-id');
      const nome = prompt('Nome do vendedor (aparecerá para o comprador):');
      if(!nome) return;
      const whats = prompt('WhatsApp do vendedor (somente números, DDD):');
      if(!whats) return;
      try{ btn.disabled=true; await api(`/api/campanhas/${id}/vendedores`, { method:'POST', auth:true, body:{ nome: nome.trim(), whatsapp: normalizeWhats(whats) } }); toast('Vendedor cadastrado','success'); }
      catch(e){ toast(e.message||'Erro ao cadastrar vendedor','error'); }
      finally{ btn.disabled=false; }
    }));
  }
}
...
async function pageCampanha(){
  ...
  // vendedores: público (comprador precisa escolher obrigatoriamente)
  let sellers=[];
  try{ sellers = await api(`/api/campanhas/${id}/vendedores`,{auth:false}); }
  catch{ sellers=[]; }
        ...
        <div class="form">
          ...
          <div class="row2">
            <div>
              ...
              ${sellers.length ? `
                <select id="p_seller" required>
                  <option value="">Selecione...</option>
                  ${sellers.map(s=>`<option value="${s.id}">${escapeHtml(s.nome)} (${escapeHtml(s.whatsapp)})</option>`).join("")}
                </select>
              ` : `
                <div class="notice">Sem vendedores cadastrados nesta campanha. O ADMIN MASTER precisa cadastrar vendedores antes das reservas.</div>
                <select id="p_seller" disabled><option>Indisponível</option></select>
              `}
            </div>
            <div style="display:flex;align-items:flex-end;gap:10px">
              <button class="btn primary" id="p_reservar" ${sellers.length?"":"disabled"}>Reservar</button>
  ...
  if(btn){
    btn.onclick=async ()=>{
      ...
      try{
        const sellerVal = Number(sellers.length ? qs("#p_seller").value : qs("#p_seller").value.trim());
        if(!sellerVal) throw new Error("Selecione um vendedor");
