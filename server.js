...
async function initDB() {
  ...
  // migrações leves para permitir vendedores sem conta
  try{ await q(`ALTER TABLE campaign_sellers ALTER COLUMN seller_user_id DROP NOT NULL;`);}catch{}
  try{ await q(`ALTER TABLE campaign_sellers ADD COLUMN IF NOT EXISTS seller_nome TEXT;`);}catch{}
  try{ await q(`ALTER TABLE campaign_sellers ADD COLUMN IF NOT EXISTS seller_whatsapp TEXT;`);}catch{}

  try{ await q(`ALTER TABLE campaign_quotas ADD COLUMN IF NOT EXISTS sold_by_seller_id INTEGER;`);}catch{}
  try{ await q(`ALTER TABLE campaign_quotas ADD COLUMN IF NOT EXISTS sold_by_seller_nome TEXT;`);}catch{}
  try{ await q(`ALTER TABLE campaign_quotas ADD COLUMN IF NOT EXISTS sold_by_seller_whatsapp TEXT;`);}catch{}
...
app.post("/api/campanhas/:id/vendedores", authRequired, requireRole("ADMIN_MASTER"), async (req, res) => {
  // v2: vendedores podem ser cadastrados por nome/whatsapp (sem precisar ter conta)
  const schema = z.object({
    nome: z.string().min(2),
    whatsapp: z.string().min(8),
    seller_user_id: z.coerce.number().int().positive().optional(),
  });
  try{
    const data = schema.parse(req.body||{});
    const r = await q(
      `INSERT INTO campaign_sellers (campaign_id,seller_user_id,seller_nome,seller_whatsapp)
       VALUES ($1,$2,$3,$4)
       RETURNING id, COALESCE(seller_nome,'') as nome, COALESCE(seller_whatsapp,'') as whatsapp`,
      [Number(req.params.id), data.seller_user_id || null, data.nome, data.whatsapp]
    );
    res.json({ ok:true, seller: r.rows[0] });
  } catch(e){
    res.status(400).json({ message: e?.issues?.[0]?.message || "Dados inválidos" });
  }
});

// público: o comprador deve conseguir escolher um vendedor
app.get("/api/campanhas/:id/vendedores", async (req,res)=>{
  const rows = await q(
    `SELECT id,
            COALESCE(seller_nome,'') as nome,
            COALESCE(seller_whatsapp,'') as whatsapp
     FROM campaign_sellers
     WHERE campaign_id=$1
     ORDER BY seller_nome ASC`,
...
app.post("/api/campanhas/:id/reservar", authRequired, async (req,res)=>{
  ...
  try{
    ...
    const sellerOk = await q(
      `SELECT id, COALESCE(seller_nome,'') as nome, COALESCE(seller_whatsapp,'') as whatsapp, seller_user_id
       FROM campaign_sellers WHERE campaign_id=$1 AND id=$2`,
      [campId, data.seller_id]
    );
    if (!sellerOk.rows.length) return res.status(400).json({ message: "Vendedor inválido para esta campanha" });
    const seller = sellerOk.rows[0];
    ...
    try{
      ...
      await client.query(`
        ...
        SET status='RESERVADA', reserved_by_user_id=$1, reserved_at=NOW(),
            sold_by_seller_user_id=$2,
            sold_by_seller_id=$6, sold_by_seller_nome=$7, sold_by_seller_whatsapp=$8,
            buyer_nome=$3, buyer_whatsapp=$4
        WHERE id=$5
      `,[
        req.user.id,
        seller.seller_user_id || null,
        data.buyer_nome,
        normalizeWhatsApp(data.buyer_whatsapp),
        quota.rows[0].id,
        seller.id,
        seller.nome,
        seller.whatsapp,
